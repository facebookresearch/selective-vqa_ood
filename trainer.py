...
class Trainer(object):
    ...
    def _build_ema(self):
        if getattr(self.task, "ema_selector_only", False):
            parameters = ["selective_predictor." + name for name, param in self._model.selective_predictor.named_parameters()]
            logger.info(
                "Exponential Moving Average Shadow Model is initialized with subset of paraterers " + str(parameters)
            )
        else:
            parameters=None

        if self.cfg.ema.store_ema:
            self._ema = build_ema(self._model, self.cfg.ema, self.device, parameters=parameters)
            logger.info(
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                if (not is_resume) and self.cfg.checkpoint.use_ema_weights_to_init_param and "extra_state" in state and "ema" in state["extra_state"]:
                    ...
                    if "no_strict_load" in self.cfg.model:
                        strict = not self.cfg.model.no_strict_load
                    else:
                        strict = True
                    ...
                    print("Missing kes", missing_keys)
        ...
        results = list(
            zip(
                *distributed_utils.all_gather_list(
                    ...
                    max_size=getattr(self.cfg.common, "all_gather_list_size", 2*16384),
                    group=self.data_parallel_process_group,
                )
            )
        )
