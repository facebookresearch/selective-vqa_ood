...
class VqaGenTask(OFATask):
    def __init__(self, cfg: VqaGenConfig, src_dict, tgt_dict):
        ...
        assert self.cfg.val_inference_type in [
            "allcand",
            "beamsearch",
    ...
    def decode_tokens(self, tokens, ignore_symbols=False):
        if ignore_symbols:
            extra_symbols_to_ignore = None
        else:
            extra_symbols_to_ignore = get_symbols_to_strip_from_output(self.generator)
        x = self.tgt_dict.string(
            tokens.int().cpu(), extra_symbols_to_ignore=extra_symbols_to_ignore
        )
    ...
    def build_model(self, cfg):
        ...
        if self.cfg.val_inference_type == "allcand":
            ...
            self.generator = self.build_generator([model], Namespace(**gen_args))
        else:
        ...
        with torch.no_grad():
            if self.cfg.val_inference_type == "allcand":
                ...
                for valid_answers, valid_constraint_masks in zip(
                    ...
                    lprobs = eval_model.get_normalized_probs(
                        decoder_out, log_probs=True
                    )
                    scores = lprobs.gather(
                        dim=-1, index=valid_tgt.unsqueeze(-1)
                    ).squeeze(-1)
                ...
                for i, sample_id in enumerate(sample["id"].tolist()):
                    ...
                    detok_hypo_str = decode_fn(
                        raw_hyps[i][0]["tokens"][prefix_len:],
                        self.tgt_dict,
                        self.bpe,
                        self.generator,
                    )
                    score = raw_hyps[i][0]["positional_scores"][prefix_len:].sum()
                    probs.append(torch.exp(score))
