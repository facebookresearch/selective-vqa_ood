...
from fairseq.data import FairseqDataset, iterators, data_utils
...
class OFAConfig(FairseqDataclass):
    ...
    random_batch: Optional[bool] = field(
        default=False,
        metadata={"help": "random batch"}
    )
...
class OFATask(FairseqTask):
    ...
    def get_batch_iterator(
        ...
        batch_sampler = [
            [indices[j] for j in range(i, min(i + max_sentences, len(dataset)))]
    ...
    def build_generator(
        ...
        return seq_gen_cls(
            ...
            temperature_prob=getattr(args, "temperature_prob", 1.0),
            match_source_len=getattr(args, "match_source_len", False),
