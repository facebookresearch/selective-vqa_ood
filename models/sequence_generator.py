...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        self.temperature_prob = temperature_prob
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            with torch.autograd.profiler.record_function("EnsembleModel: forward_decoder"):
                ...
                if  self.temperature_prob is not None:
                    lprobs, avg_attn_scores, lprobs_temp = res
                else:
                    lprobs, avg_attn_scores = res
                    lprobs_temp = lprobs
                ...
                finalized[sent_list[i]].append(
                    {
                        ...
                        "positional_scores": pos_scores[i],  # Here i should add the confidence
                        "confidence": None,
                    }
                )
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if temperature_prob is not None:
                decoder_out_tuple_temp = (decoder_out[0][:, -1:, :].div(temperature_prob),)
            else:
                decoder_out_tuple_temp = None
            ...
            if constraint_trie is not None and not zero_shot:
                ...
                if decoder_out_tuple_temp is not None:
                    decoder_out_tuple_temp[0].masked_fill_(~constraint_masks, -math.inf)
            if constraint_start is not None and constraint_end is not None and not zero_shot:
                ...
                if decoder_out_tuple_temp is not None:
                    decoder_out_tuple_temp[0][:, :, 4:constraint_start] = -math.inf
                    decoder_out_tuple_temp[0][:, :, constraint_end:] = -math.inf

            probs = model.get_normalized_probs(
                decoder_out_tuple, log_probs=True, sample=None
            )
            if decoder_out_tuple_temp is not None:
                probs_with_temp = model.get_normalized_probs(
                    decoder_out_tuple_temp, log_probs=True, sample=None
                )
            if constraint_trie is not None and zero_shot:
                ...
                if temperature_prob is not None:
                    probs_with_temp.masked_fill_(~constraint_masks, -math.inf)
            if constraint_start is not None and constraint_end is not None and zero_shot:
                ...
                if temperature_prob is not None:
                    probs_with_temp[:, :, 4:constraint_start] = -math.inf
                    probs_with_temp[:, :, constraint_end:] = -math.inf
            probs = probs[:, -1, :]
            if temperature_prob is not None:
                probs_with_temp = probs_with_temp[:, -1, :]

            if self.models_size == 1:
                if temperature_prob is not None:
                    return probs, attn, probs_with_temp
                return probs, attn
