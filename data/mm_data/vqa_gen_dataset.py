...
def collate(samples, pad_idx, eos_idx):
 ...
 # This is for predictions and scores that are already extracted; for selector training
    if samples[0].get("prediction", None) is not None:
        batch["prediction"] = np.array([s['prediction'] for s in samples])
        batch["score"] = np.array([s['score'] for s in samples])
...
class VqaGenDataset(OFADataset):
    ...
    def __getitem__(self, index):
        ...
        if len(item) == 5:
            ...
            uniq_id, image, question, ref, predict_objects, pred, score = item
        else:
        ...
        elif image.startswith("/datasets01/"):
            image = Image.open(image)
        else:
        ...
        ref_dict = {item.split('|!+')[1]: float(item.split('|!+')[0]) for item in ref.rsplit('&&')}
