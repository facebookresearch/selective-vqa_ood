...
class EMASubset(object):
    ...
    def __init__(self, model, config, device=None, parameters=None):
        """
        ...
        @param parameters: list of parameters for which the EMA has to be applied.
    ...
    def _step_internal(self, new_model, updates=None):
        ...
        for key, param in new_model.state_dict().items():
            if self.parameters is not None and key not in self.parameters:
                # We do not update this parameter
                continue
            try:
