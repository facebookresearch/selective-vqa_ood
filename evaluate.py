...
def main(cfg: DictConfig, **kwargs):
    ...
    # saved_cfg.task["load_all_data"] = cfg.task.get("load_all_data", False)

    # loading the dataset should happen after the checkpoint has been loaded so we can give it the saved task config
    task.load_dataset(cfg.dataset.gen_subset, task_cfg=saved_cfg.task, new_cfg=cfg)
    ...
    for sample in progress:
        ...
        with torch.no_grad():
            if kwargs["zero_shot"]:
                result, scores = zero_shot_step(task, generator, models, sample, **kwargs)
            else:
...
if __name__ == "__main__":
    ...
    # executor.update_parameters(timeout_min=300, slurm_partition="devlab")
    # executor.submit(cli_main)
    cli_main()
