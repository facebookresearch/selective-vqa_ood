...
# ========
# Below is the original OFA copyright notification.
# Note: The "LICENSE" referred to in OFA's notice is under "OFA_LICENSE" in this repo.
...
def eval_vqa_gen(task, generator, models, sample, **kwargs):
    if kwargs['beam_search_vqa_eval']:
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            probs = hypos[i][0]["positional_scores"][prefix_len:].sum().exp()
            results.append({
...
def eval_vqa_select(task, generator, models, sample, **kwargs):
    if kwargs['beam_search_vqa_eval']:
        hypos = task.inference(models[0], sample)
        results = []

        for i, sample_id in enumerate(sample["id"].tolist()):
            hyp, prob, conf, score = hypos[i]
            results.append({
                "question_id": int(sample_id), 
                "answer": hyp,
...
def eval_domain_classif(task, generator, models, sample, **kwargs):
    if kwargs['beam_search_vqa_eval']:
        hypos = task.inference_step(generator, models, sample, prefix_tokens=sample['prefix_tokens'])
        results = []
        for i, sample_id in enumerate(sample["id"].tolist()):
    ...
    predicts = valid_result.argmax(1).tolist()
    hyps = [task.index2ans[predict_index] for predict_index in predicts]
    probs = valid_result.exp()
    probs = [probs[i][0].item() for i in range(len(valid_result))]
    results = [{"question_id": int(id), "answer": hyp, "prob": prob} for id, hyp, prob in zip(sample["id"].tolist(), hyps, probs)]
    scores = [ref_dict.get(hyp, 0) for ref_dict, hyp in zip(sample['ref_dict'], hyps)]
...
def eval_step(task, generator, models, sample, **kwargs):
    ...
    elif task.cfg._name == "vqa_select_pred":
        return eval_vqa_select(task, generator, models, sample, **kwargs)
    elif task.cfg._name == "vqa_classif_domain":
        return eval_domain_classif(task, generator, models, sample, **kwargs)
...
def merge_results(task, cfg, logger, score_cnt, score_sum, results):
    if task.cfg._name == 'image_gen':
        ...
        if score_cnt.item() > 0:
            score = round(score_sum.item() / score_cnt.item(), 4)
            logger.info("score_sum: {}, score_cnt: {}, score: {}".format(
                score_sum, score_cnt, score
            ))
        
        # Save features if we need to
        if hasattr(task, "feature_saver"):
            save_feature_path = get_arg(task.generator_args, "save_features_path", None)
            if save_feature_path is not None:
                task.feature_saver.save(save_feature_path)

        # Save results
        if cfg.distributed_training.distributed_world_size == 1 or dist.get_rank() == 0:
            result_output_path = os.path.join(cfg.common_eval.results_path, "{}_predict.json".format(cfg.dataset.gen_subset))
            gather_results = list(chain(*gather_results)) if gather_results is not None else results
            with open(result_output_path, 'w') as fw:
                json.dump(gather_results, fw)

            score_dict = {"score": score}

            # Save score
            output_path = os.path.join(cfg.common_eval.results_path, "score.json")
            with open(output_path, "w") as f:
                json.dump(score_dict, f)

            # compute AUC
            if "confidence" in gather_results[0]:
                metric = RiskCoverage(gather_dist=False)
                confidences = np.array([res["confidence"] for res in gather_results])
                scores =  np.array([res["score"] for res in gather_results])
                metric.add(confidences, scores)
                res = metric.compute()
                for key in res:
                    score_dict[key] = res[key]
